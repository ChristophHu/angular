...
@Component({
  ...
  styleUrls: ['./bar-chart-vertical.component.sass']
})
export class BarChartVerticalComponent implements AfterViewInit, OnChanges {
  ...
  private createChart(): void {
    ...
    if (this.isDataValid()) {
      ...
      let barRects = barGroup.selectAll<SVGRectElement, {name: string, value: number}>('rect.bar').data((d: any) => d.series.map((item: any) => item));
      barRects.enter()
        .append('rect')
        .merge(barRects)
        .attr('class', 'bar')
        ...
        .attr('fill', (d: any, i: any) => this.barColors[i]);

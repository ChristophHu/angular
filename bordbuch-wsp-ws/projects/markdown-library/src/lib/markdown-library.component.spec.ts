...
import { KatexOptions, MarkdownLibraryService } from './markdown-library.service'

describe('MarkdownLibraryComponent', () => {
  let fixture: ComponentFixture<MarkdownLibraryComponent>
  let component: MarkdownLibraryComponent
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(MarkdownLibraryComponent)
  ...
  describe('src', () => {
    ...
    it('should call render with retreived content when set', waitForAsync(() => {
      ...
      spyOn(markdownLibraryService, 'getSource').and.returnValue(of(mockContent))

      component.src = mockSrc

      component.ngOnChanges()

      expect(markdownLibraryService.getSource).toHaveBeenCalledWith(mockSrc)
    ...
    it('should return value correctly when get', () => {
      ...
      spyOn(markdownLibraryService, 'getSource').and.returnValue(of())
    ...
    it('should emit load when get', () => {
      ...
      spyOn(markdownLibraryService, 'getSource').and.returnValue(of(mockSrcReturn))
    ...
    it('should emit error when and error occurs', () => {
      ...
      spyOn(markdownLibraryService, 'getSource').and.returnValue(throwError(mockError))
  ...
  describe('render', () => {
    ...
    it('should set innerHTML with compiled/decoded html markdown when decodeHtml is true', () => {
      ...
      spyOn(markdownLibraryService, 'compile').and.callFake((markdown: string, decodeHtml: boolean, emojify: boolean) => {
        return decodeHtml ? decoded : ''
      })

      component.render(raw, true)

      expect(markdownLibraryService.compile).toHaveBeenCalledWith(raw, true, false)
    ...
    it('should set innerHTML with compiled/undecoded html markdown when decodeHtml is omitted/false/null/undefined', () => {
      ...
      expect(markdownLibraryService.compile).toHaveBeenCalledWith(raw, false, false)
    ...
    it('should set innerHTML with compiled/decoded html markdown', () => {
      ...
      spyOn(markdownLibraryService, 'compile').and.returnValue(compiled)
    ...
    it('should apply highlight', () => {
      ...
      spyOn(markdownLibraryService, 'highlight')

      component.render('### Raw')

      expect(markdownLibraryService.highlight).toHaveBeenCalledWith(component.element.nativeElement)
    })
    ...
    it('should apply emoji plugin correctly', () => {
      ...
      expect(markdownLibraryService.compile).toHaveBeenCalledWith(raw, false, true)
    ...
    it('should apply katex plugin correctly', () => {
      ...
      spyOn(markdownLibraryService, 'compile').and.returnValue(compiled)
      spyOn(markdownLibraryService, 'renderKatex')

      component.katex = true
      component.katexOptions = katexOptions
      component.render(markdown)

      expect(markdownLibraryService.renderKatex).toHaveBeenCalledWith(compiled, katexOptions)
    })
    ...
    it('should emit `ready` when done parsing', waitForAsync(() => {
      ...
      spyOn(markdownLibraryService, 'compile').and.returnValue(compiled)
      spyOn(markdownLibraryService, 'renderKatex').and.returnValue(compiled)
      spyOn(markdownLibraryService, 'highlight')

      component.ready
        .pipe(first())
        .subscribe(() => {
          expect(markdownLibraryService.compile).toHaveBeenCalled()
          expect(markdownLibraryService.renderKatex).toHaveBeenCalled()
          expect(markdownLibraryService.highlight).toHaveBeenCalled()
          expect(component.element.nativeElement.innerHTML).toBe(compiled)
        })
